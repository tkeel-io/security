...
func MappingFromExternal(sub, name, email, tenantID string) (*User, error) {
	tenantIDInt, err := strconv.ParseInt(tenantID, 10, 32)
	if err != nil {
		return nil, fmt.Errorf("user mapping from external %w", err)
	}
	users := make([]*User, 0)
	condition := map[string]interface{}{
		"tenant_id":   int(tenantIDInt),
		"external_id": sub,
	}
	err = _db.Model(&User{}).Where(condition).Find(&users).Error
	if err != nil {
	...
	if len(users) == 0 {
		user := &User{
			ID:         GenUserID(int(tenantIDInt)),
			UserName:   name,
			ExternalID: sub,
			Email:      email,
			TenantID:   int(tenantIDInt),
		}
		err = user.Create()
		return user, err
	}
